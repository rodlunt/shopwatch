"""Auto-deriving trigger_price/excellent_price from lowest_known_price (issue #101).

A confirmed lowest_known_price should not leave a product un-actionable just because
nobody typed a trigger or excellent price in by hand. store.maybe_derive_price_targets
fills either target that is still NULL, and never touches one that already has a value -
the same "a manual edit beats an automated write" discipline provenance.py already
enforces for listings, just as a plain boolean per field (migrations/0014) rather than
the full state machine, since a product only ever has one auto-writer for these two
fields.
"""
...
def test_derives_both_targets_from_lowest_known_price(conn):
...
def test_never_overrides_a_value_the_user_set_by_hand(conn):
    product_id = store.create_product(conn, {
        ...
        "trigger_price": 1500.0, "excellent_price": 1200.0,
    })
    store.update_product(conn, product_id, {"lowest_known_price": 1000.0})
    conn.commit()
...
def test_fills_only_the_target_left_unset(conn):
...
def test_hand_typed_change_locks_the_field(conn):
    ...
    # A real edit: a different number than the auto-filled one.
    store.update_product(conn, product_id, {"trigger_price": 1300.0})
    conn.commit()
    product = store.get_product(conn, product_id)
    assert product["trigger_price"] == 1300.0
    assert product["trigger_price_auto"] == 0

    # A lower low afterwards must not overwrite the now-locked trigger.
...
def test_clearing_a_target_unlocks_it_and_refills_immediately(conn):
    product_id = store.create_product(conn, {
        ...
        "lowest_known_price": 1000.0, "trigger_price": 1300.0,
    })
    conn.commit()
    product = store.get_product(conn, product_id)
    assert product["trigger_price_auto"] == 0  # typed in at creation, not derived

    store.update_product(conn, product_id, {"trigger_price": None})
    conn.commit()
