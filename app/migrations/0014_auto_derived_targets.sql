-- 0014: mark trigger_price/excellent_price as system-derived versus hand-typed
-- (issue #101).
--
-- Both columns already exist and default to NULL, meaning "no target set". These two
...
-- filled it in from lowest_known_price. That distinction only matters for display (the
...
-- product has exactly one auto-writer for these two fields and the only question ever
